...
class SaleOrderLine(models.Model):
    ...
    configurator_mode = fields.Selection(
        [("matrix", "Matrix Grid"), ("configurator", "Product Configurator")],
        string="Config Mode",
        default="matrix",
    ...
    configurator_mode_manual = fields.Boolean(
        default=False, help="Technical field to track manual mode selection"
    )

    @api.onchange("product_template_id")
    def _onchange_product_template_configurator_mode(self) -> None:
        """Auto-select mode based on product configuration only if not manually set"""
        # Skip configurator logic for section and note lines
    ...
    def _onchange_configurator_mode(self) -> None:
    ...
    def copy(self, default: dict[str, Any] | None = None) -> models.Model:
