...
class ProductTemplate(models.Model):
    ...
    def _compute_estimated_landed_cost(self):
        for template in self:
            duty_percent = template.intrastat_duty_id.duty if template.intrastat_duty_id else 0.0
            template.estimated_landed_cost = template.standard_price * (1 + duty_percent / 100)
