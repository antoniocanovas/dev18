...
class StockPicking(models.Model):
    ...
    def button_validate(self):
        ...
        for picking in self:
            ...
            all_manufacturer_locations = self.env['shoes.accesory'].search([]).mapped('manufacturer_location_id')
            if picking.location_id not in all_manufacturer_locations:
                continue

            # Find the virtual production location
            production_location = self.env.ref('stock.stock_location_production', raise_if_not_found=False)
            ...
            for move in picking.move_ids_without_package:
                ...
                accessories = self.env['shoes.accesory'].search([('task_id', '=', move.product_id.shoes_task_id.id)])
