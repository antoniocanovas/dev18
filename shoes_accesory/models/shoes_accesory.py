...
class ShoesAccesory(models.Model):
    ...
    manufacturer_id = fields.Many2one(
        "res.partner", store=True, related="task_id.manufacturer_id"
    )
    manufacturer_location_id = fields.Many2one(
        "stock.location", store=True, related="manufacturer_id.property_stock_customer"
    )
    shoes_campaign_id = fields.Many2one(
        "project.project", string="Campaign", store=True, related="task_id.project_id"
    )

    # Computed:
    pairs_campaign_sold = fields.Integer(
        "Sold pairs", compute="_compute_pairs_campaign_sold"
    )
    pairs_campaign_pending = fields.Integer(
        "Pending pairs", compute="_compute_pairs_campaign_pending"
    )
    accesory_campaign_sold_qty = fields.Float(
        "Sold accesories", compute="_compute_accesory_campaign_sold_qty"
    )
    accesory_campaign_pending_qty = fields.Float(
        "Pending accesories", compute="_compute_accesory_campaign_pending_qty"
    )
    accesory_manufacturer_stock = fields.Float(
        "Manufacturer stock",
        help="Enviados menos producidos",
        compute="_compute_accesory_manufacturer_stock",
    )
    accesory_stock = fields.Float(
        "Stock", related="product_id.qty_available", store=True
    )
    accesory_supplier_pending = fields.Float(
        "Assigned", compute="_compute_accesory_supplier_pending"
    )
    ...
    def _compute_accesory_campaign_pending_qty(self):
        for record in self:
            record.accesory_campaign_pending_qty = (
                record.pairs_campaign_pending * record.qty
            )
    ...
    def _compute_accesory_supplier_pending(self):
        ...
        for record in self:
            ...
            record.accesory_supplier_pending = sum(
                assigned_moves.mapped("product_uom_qty")
            )

    @api.depends(
        "product_id", "task_id", "manufacturer_id", "manufacturer_location_id", "qty"
    )
    def _compute_accesory_manufacturer_stock(self):
        ...
        for record in self:
            if not all(
                [
                    record.product_id,
                    record.manufacturer_location_id,
                    record.task_id,
            ...
            # 1. Find the date of the last inventory adjustment for the product in the
            ...
            last_inventory_move = StockMove.search(
                [
                    ...
                    "|",
                    "&",
                    ("location_id", "=", record.manufacturer_location_id.id),
            ...
            last_inventory_date = (
                last_inventory_move.date or fields.Datetime.from_string("1970-01-01")
            )
            ...
            for line in purchase_lines:
                ...
                line_moves = line.move_ids.filtered(
                    lambda m, last_inventory_date=last_inventory_date: m.state
                    == "done"
                    and m.date > last_inventory_date
                )
                qty_received_since_inventory = sum(line_moves.mapped("quantity"))
                if qty_received_since_inventory > 0:
                    total_pairs_received_since_inventory += (
                        qty_received_since_inventory * line.product_id.pairs_count
                    )

            consumed_qty_since_inventory = (
                total_pairs_received_since_inventory * record.qty
            )

            # 4. Final calculation
            record.accesory_manufacturer_stock = (
                sent_qty_since_inventory - consumed_qty_since_inventory
            )

    @api.depends(
        "accesory_campaign_pending_qty",
        "accesory_manufacturer_stock",
        "accesory_supplier_pending",
    )
    def _compute_accesory_manufacturer_status(self):
        for record in self:
            record.accesory_manufacturer_status = (
                record.accesory_manufacturer_stock + record.accesory_supplier_pending
            ) - record.accesory_campaign_pending_qty
