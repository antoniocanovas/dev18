...
class ProductAttributeValue(models.Model):
    ...
    # Calcula si el atributo de conjunto debe estar oculto basado en la configuración
    # de la compañía
    def _get_set_hidden(self):
        company_bom_attribute = self.env.user.company_id.assortment_attribute_id
        set_hidden = True
        if (company_bom_attribute.id) and (
            company_bom_attribute.id == self.attribute_id.id
        ):
            set_hidden = False
    ...
    def _constrains_no_custom_in_sizes_colors(self):
        for record in self:
            ...
            if record.is_custom and record.attribute_id.id in [
                color_attribute.id,
                size_attribute.id,
            ]:
                raise UserError(
                    "Custom color and size values are not allowed, please review."
                )
