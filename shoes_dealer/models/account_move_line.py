...
class AccountMoveLine(models.Model):
    ...
    pairs_count = fields.Integer(
        "Pairs", store=True, compute="_get_shoes_invoice_line_pair_count"
    )
    ...
    def _get_shoes_invoice_pair_price(self):
        for record in self:
            record.pair_price = (
                record.price_subtotal / record.pairs_count if record.pairs_count else 0
            )

    pair_price = fields.Float(
        "Pair price", store=True, compute="_get_shoes_invoice_pair_price"
    )
    ...
    shoes_model_id = fields.Many2one(
        "product.template", store=True, related="product_id.shoes_model_id"
    )
    exwork_single_euro = fields.Monetary(related="shoes_model_id.exwork_single_euro")

    # Campo manufacturer_id con store=True para uso en informes SQL
    #
    manufacturer_id = fields.Many2one(
        ...
        store=True,
        help="Used for group by manufacturer in invoice reports",
    )
    ...
    shoes_pair_margin = fields.Monetary(
        "Pair margin", store=True, compute="_get_shoes_pair_margin"
    )
    ...
    pair_price_sale = fields.Monetary(
        "Pair price sale", store=True, compute="_get_pair_price_sale"
    )
    ...
    def _get_cost_price(self):
        for record in self:
            record.cost_price = (
                record.pairs_count * record.exwork_single_euro
    ...
    def _get_total_shoes_discount(self):
        for record in self:
            # Chequeo de si es factura de cliente o abono:
            type = 1 if record.move_type == "out_invoice" else -1
            record.discount_amount = type * (
                record.price_unit * record.quantity - record.price_subtotal
            )

    discount_amount = fields.Monetary(
        "Total discount", store=True, compute="_get_total_shoes_discount"
    )
