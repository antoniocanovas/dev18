...
class ShoesAnalysis(models.Model):
    ...
    def action_export_to_spreadsheet(self):
        ...
        if not config:
            raise UserError(
                f"La exportación a hoja de cálculo no está implementada para el tipo de informe: {self.type}"
            )
        ...
        return {
            ...
            "res_model": export_model._name,  # Usar el nombre del modelo
            # transitorio único
            "view_mode": "list",
        ...
        for salesman_data in json_data.get("detalle_representantes", []):
            for campaign_data in salesman_data.get("campanias", []):
                lines_to_create.append(
                    {
                        "referrer_id": self.env["res.users"]
                        .search(
                            [("name", "=", salesman_data.get("representante"))], limit=1
                        )
                        .id,
    ...
    def _parse_salesman_country_data(self, json_data):
        ...
        for (
            ...
            referrer_id = (
                self.env["res.users"]
                .search([("name", "=", salesman_data.get("salesman_name"))], limit=1)
                .id
            )
            for country_data in salesman_data.get("countries", []):
                country_id = (
                    self.env["res.country"]
                    .search([("name", "=", country_data.get("country_name"))], limit=1)
                    .id
                )
    ...
    def _parse_manufacturer_sales_data(self, json_data):
        ...
        for (
            ...
            manufacturer_id = (
                ...
                .search(
                    [("name", "=", manufacturer_data.get("manufacturer_name"))], limit=1
                )
                .id
            )
    ...
    def _parse_sales_by_shipping_mark_data(self, json_data):
        ...
        for mark_data in json_data:  # La estructura JSON es una lista de timbrados
            sale_type_id = (
                ...
                .search([("name", "=", mark_data.get("shipping_mark_name"))], limit=1)
                .id
            )
