...
class ShoesAnalysis(models.Model):
    ...
    def _generate_line_html(self, data_dict, base_camp_id, currency_obj):
        ...
        html_parts = [f"<div style='border: 2px solid #333; border-radius: 5px; margin-bottom: 30px; padding: 20px; background-color: #f0f0f0;'>"]
        ...
        html_parts.append('<table class="table table-sm o_main_table" style="width: 100%; table-layout: fixed;">')
        html_parts.append(f'<thead><tr style="font-size: 0.85em; color: #555;"><th style="{style_camp}">Campaña</th><th class="text-end" style="{style_total_sold}">Total Vend.</th><th class="text-end" style="{style_total_cancel}">Total Canc.</th><th class="text-end" style="{style_net}">Netos</th><th class="text-end" style="{style_asign}">Asignados</th><th class="text-end" style="{style_perc_asign}">% Asign.</th><th class="text-end" style="{style_delivered}">Servidos</th><th class="text-end" style="{style_perc_serv}">% Servidos</th><th class="text-end" style="{style_pending}">Pendientes</th><th class="text-end" style="{style_revenue}">Fact. Prevista</th><th class="text-end" style="{style_perc_obj}">%Pares</th><th class="text-end" style="{style_perc_obj}">%Ventas</th></tr></thead>')
        html_parts.append('<tbody>')
        
        html_parts.append(f'<tr><td style="{style_camp}"><b>{html_escape(base_data.get("campania_nombre", "N/A"))} (Actual)</b></td><td class="text-end" style="{style_total_sold}"><b>{base_data.get("total_vendidos", 0)} Pairs</b></td><td class="text-end" style="{style_total_cancel}"><b>{base_data.get("total_cancelados", 0)} Pairs</b></td><td class="text-end" style="{style_net}"><b>{base_netos} Pairs</b></td><td class="text-end" style="{style_asign}"><b>{base_asignados} Pairs</b></td>{self._get_asign_perc_html(base_asignados, base_netos, style_perc_asign)}<td class="text-end" style="{style_delivered}"><b>{base_delivered} Serv.</b></td>{self._get_servidos_perc_html(base_delivered, base_netos, style_perc_serv)}<td class="text-end" style="{style_pending}"><b>{base_data.get("pairs_pending", 0)} Pend.</b></td><td class="text-end" style="{style_revenue}"><b>{formatLang(self.env, base_data.get("expected_revenue", 0.0), currency_obj=currency_obj)}</b></td><td class="text-end" style="{style_perc_obj}">-</td><td class="text-end" style="{style_perc_obj}">-</td></tr>')
