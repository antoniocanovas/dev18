...
class ShoesAnalysis(models.Model):
    ...
    def _compute_manufacturer_sales(self):
        ...
        for group in sales_data:
            ...
            data_map[manufacturer_id][campaign_id]['net_pairs'] += net_pairs
            data_map[manufacturer_id][campaign_id]['net_sales'] += net_sales
        ...
        for manufacturer in manufacturers:
            ...
            manufacturer_html.append(f"<table class='table table-sm'><thead><tr><th style='{style_th}'>Campaña</th><th class='text-end' style='{style_th}'>Pares Netos</th><th class='text-end' style='{style_th}'>Ventas Netas</th><th class='text-end' style='{style_th}'>% Obj. Pares</th><th class='text-end' style='{style_th}'>% Obj. Ventas</th></tr></thead><tbody>")
            ...
            for camp_id, stats in campaigns_data.items():
                ...
                row_html += "</tr>"
                manufacturer_html.append(row_html)
                
                manufacturer_json['campaigns'].append({'campaign_id': camp.id, 'campaign_name': camp.name, 'net_pairs': stats['net_pairs'], 'net_sales': stats['net_sales']})

            manufacturer_html.append("</tbody></table></div>")
            html_parts.extend(manufacturer_html)
            json_output.append(manufacturer_json)
